...
| 5 | Collect sample runbooks, postmortems, code docs for the RAG corpus | Corpus covers all 6 sample queries, especially connection-pool exhaustion | ✅ **Done — reformatted** | `src/data/corpus/` — 5 runbooks, 3 postmortems, 2 code docs, plus `sources.md` mapping every doc to the 6 sample queries in `requirements.md` §3. Ashish subsequently restructured every document (2026-07-15) with a `\| Field \| Value \|` metadata table and standardized headings (runbooks: Overview → Symptoms → Diagnosis → Mitigation → Prevention → Related Runbooks) — see `docs/decision-log.md` ADR-11. |
| 6 | Build the ingestion pipeline: chunk and embed docs into a vector store | Pipeline runs with no errors; vector store has expected chunk count | ⬜ **Not Started** | No ingestion code, no vector-store dependency in `pyproject.toml`. **Design note:** the corpus's new standardized section headings (ADR-11) are a natural chunk boundary — chunking by section rather than by fixed character count will likely retrieve more precisely. |
